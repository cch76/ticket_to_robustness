...
import models.cifar.large_resnet as larsnet
import CBAM.resnet_cbam as cbam
...
def main():
    ...
    model, model_ref = model_select(args)
    model.cuda()
    model_ref.cuda()
...
def model_select(args):
    ...
    elif args.arch.endswith('resnet34'):
        model = larsnet.ResNet34(num_classes=num_classes)
        model_ref = larsnet.ResNet34(num_classes=num_classes)

    elif args.arch.endswith('wrn34'):
        print('wrn')
        model = larsnet.wrn34(num_classes=num_classes,
                        widen_factor=2)
        model_ref = larsnet.wrn34(num_classes=num_classes,
                        widen_factor=2)

    elif args.arch.endswith('cbam'):
        print('cbam')
        model = cbam.ResidualNet( 'CIFAR10', args.depth, 10, args.att_type)
        model_ref = cbam.ResidualNet( 'CIFAR10', args.depth, 10, args.att_type)
